...
describe("POST /api/progress/complete-practice", () => {
  it("refunds quota and returns 503 when the runner is unavailable", async () => {
    ...
    const response = await POST(
      new Request("http://localhost/api/progress/complete-practice", {
        method: "POST",
        body: JSON.stringify({ challengeId: "fizz-buzz", code: "print(1)" }),
      })
    );
